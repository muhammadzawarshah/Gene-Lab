...
import {
  ...
  ArrowUpRight, Hash, CalendarDays, CreditCard, FileText
} from 'lucide-react';
...
export default function PaymentsOutHistory() {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      // Payments OUT = PAYMENT type only
      setPayments(all.filter(p => p.payment_type === 'PAYMENT'));
    } catch {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-white/5">
          <div className="flex items-center gap-5">
            <div className="bg-rose-600/10 border border-rose-500/20 p-4 rounded-2xl">
              <ArrowUpRight className="text-rose-400" size={26} />
            </div>
            <div>
              <h1 className="text-4xl font-black text-white italic tracking-tighter uppercase leading-none">
                Payments <span className="text-rose-500">Made</span>
              </h1>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {/* Total */}
            <div className="bg-rose-500/5 border border-rose-500/20 rounded-2xl px-6 py-3">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Total Paid Out</p>
              <p className="text-xl font-black text-rose-400 italic">PKR {totalPaid.toLocaleString()}</p>
            </div>
            {/* Search */}
            <div className="relative group">
              <Search className="absolute left-4 top-3.5 text-slate-500 group-focus-within:text-rose-500 transition-colors" size={16} />
              <input
                className="bg-[#0f172a] border border-slate-800 rounded-2xl py-3 pl-11 pr-5 text-sm text-white outline-none w-72 focus:border-rose-500 transition-all"
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr>
                  ...
                  <th className={thCls}>Party / Supplier</th>
                  ...
                  <th className={thCls + " text-right text-rose-400"}>Amount</th>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr key={p.payment_id} className="hover:bg-rose-500/[0.03] transition-colors group">
                    ...
                    <td className="px-5 py-4 text-center">
                      <button
                        ...
                        className="p-2 bg-white/5 rounded-xl hover:bg-rose-600 hover:text-white transition-all"
                      >
                        <Eye size={15} />
                      </button>
                    </td>
      ...
      {selected && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#0f172a] border border-white/10 rounded-[2rem] w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-rose-500/5 shrink-0">
              <div>
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => printElementById("payment-report-print", "Payment Out Report")}
