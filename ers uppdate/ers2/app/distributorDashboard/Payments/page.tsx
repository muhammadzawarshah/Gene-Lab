...
export default function PaymentsPage() {
  ...
  // 1. Cookie se raw string uthayen
  const rawCookie = Cookies.get('virtue_user');

  let currentUserId = null;

  if (rawCookie) {
    try {
      // 2. URL decoding karein (taake %22, %2C wapas symbols ban jayein)
      const decodedCookie = decodeURIComponent(rawCookie);

      // 3. String ko JSON object mein convert karein
      const userData = JSON.parse(decodedCookie);

      // 4. ID extract karein
      currentUserId = userData.id;

      console.log("Extracted User ID:", currentUserId); // Output: 5
    } catch (error) {
      console.error("Cookie parsing mein masla hai:", error);
    }
  }
  ...
  const syncPaymentVault = useCallback(async () => {
    ...
    try {
      ...
      const response = await vaultApi.get(`/api/v1/finance/specificcustomerpayment/${currentUserId}`);
      ...
      >
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <div className="px-6 py-3 bg-white/[0.02] border border-white/5 rounded-2xl">
            ...
            <p className="text-sm font-black text-blue-500 italic uppercase tracking-tighter">{currentUserId}</p>
          </div>
        </div>
      </motion.div>
                  ...
                  >
                    <div className="flex flex-wrap lg:flex-nowrap items-center justify-between gap-8">
                      <div className="flex items-center gap-6 min-w-[250px]">
                        ...
                        <div>
                          <h4 className="text-lg font-black text-white italic tracking-tighter">{pay.payment_id}</h4>
                          <div className="flex items-center gap-2 mt-1">
                            <Receipt size={10} className="text-slate-600" />
                            <span className="text-[9px] font-bold text-slate-500 uppercase">{pay.reference_number}</span>
                          </div>
                        </div>
                      </div>
