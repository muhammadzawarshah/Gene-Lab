...
import { 
  ChevronDown, CheckSquare, Loader2, Lock, Receipt, ShieldCheck
} from 'lucide-react';

// --- Types optimized for your Prisma Response ---
...
export default function CreateInvoice() {
  ...
  // --- 1. Fetch Approved GRNs for Dropdown ---
  const fetchGRNList = useCallback(async () => {
    if (!token) return;
    try {
      const res = await api.get(`/api/v1/grn/`);
      // Assuming response is an array of GRNs
      setGrnList(res.data);
    } catch (err) {
  ...
  // --- 2. Fetch GRN Details & Map to Table ---
  const fetchGRNDetails = useCallback(async (id: string) => {
    ...
    try {
      ...
      // Mapping Prisma nested structure to Flat Table structure
      const mappedItems = rawData.grnline.map((line: any) => ({
        grn_line_id: line.grn_line_id,
        product_name: line.product?.name || "Unknown",
        batch_no: line.batch?.batch_number || "No Batch",
        expiry: line.batch?.expiry_date || "N/A",
  ...
  // --- 3. Create Invoice (Only sending grn_id) ---
  const handleCreateInvoice = async () => {
    if (!selectedGRNId) return;
    const toastId = toast.loading("Processing Invoice in Backend...");
    
    try {
      // Sending ONLY grn_id as per requirement
      const payload = {
    ...
    <div className="text-white p-4 md:p-10 pb-24">
          ...
          <table className="w-full border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-blue-500/[0.02]">
                ...
                <th className={thStyle}>Batch Number</th>
              ...
              {items.length > 0 ? (
                items.map((item, idx) => (
                  <tr key={item.grn_line_id} className="hover:bg-blue-500/[0.03] transition-colors group">
                    ...
                    <td className={tdStyle}><span className="uppercase tracking-wide">{item.product_name}</span></td>
                    <td className={tdStyle + " font-mono text-slate-500"}>{item.batch_no}</td>
                    <td className={tdStyle + " font-mono text-rose-500/80 italic"}>{item.expiry || '---'}</td>
                    <td className={tdStyle}>{item.received_qty}</td>
                    <td className={tdStyle + " text-blue-400"}>{item.uom}</td>
                    <td className="p-4 text-right px-10">
                       <span className="text-[10px] font-black bg-white/5 px-4 py-2 rounded-lg text-slate-500">VERIFIED</span>
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={7} className="p-40 text-center opacity-20">
      ...
      {items.length > 0 && (
        <div className="mt-10 flex flex-col md:flex-row justify-between items-center bg-blue-500/5 p-10 rounded-[3rem] border border-blue-500/10 gap-8">
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase italic mb-1 tracking-widest">Action Required</p>
            <p className="text-3xl font-black italic text-white tracking-tighter">Ready to Generate Invoice</p>
          </div>
          <button 
          ...
          >
            <CheckSquare size={20} /> Create Invoice Now
          </button>
        </div>
      )}
    </div>
